...
template: {
	...
	parameter: {
		...
		streams: [...{
			...
			name: string
			// +usage=location defines where the stream is located. Allowed values are: node, main
			// +required
			// +default="node"
			...
			config: {
				// +usage=Streams are stored on the server, this can be one of many backends and all are usable in clustering mode. Allowed values are: file, memory
				// +default="file"
				storage?: string | *"file"
				// +usage=Messages are retained either based on limits like size and age (Limits), as long as there are Consumers (Interest) or until any worker processed them (Work Queue). Allowed values are: limits, interest, workqueue
				// +default="limits"
				...
				// +default=-1
				maxMsgSize?: int64 | *-1
				// +usage=Discard defines which messages shall be dropped. Allowed values are: old, new
				// +default="old"
				discard?: string | *"old"
			}
		}]
